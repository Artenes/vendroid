...
public class Consulta_Marcas_falsa extends Activity {
	...
	EditText edtMarca, edtFornecedor, edtCodigo;
	...
	protected void onCreate(Bundle State) {
		...
		edtMarca = (EditText) findViewById(R.id.ConsultaMarcas_edtDescricao);
			...
			sim.setOnClickListener(new View.OnClickListener() {					
				public void onClick(View v) {
					...
					if (password.moveToFirst()) {
						startActivity(new Intent(Consulta_Marcas_falsa.this,vendroid.cadastro.Cadastro_Marcas.class));
						senha.setText("");
						dialog.dismiss();
					} else {
		...
		btnPesquisar.setOnClickListener(new View.OnClickListener() {
			public void onClick(View v) {
				...
				dataSource = new SimpleCursorAdapter(contexto, R.layout.consulta_linhas,
						helper.BuscaFiltro(Codigo, Marca), campos, new int[] {R.id.ConsultaLinhas_txvCodigo,
            		R.id.ConsultaLinhas_txvDescricao});
				listview.setAdapter(dataSource);
				if (helper.BuscaFiltro(Codigo, Marca).getCount() <= 0) {
		 ...
		 listview.setOnItemClickListener(new OnItemClickListener() {
			 public void onItemClick(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				 ...
				 cur.moveToFirst();
				 String marca = cur.getString(cur.getColumnIndex("mar_descricao"));
				 Intent returIntent = new Intent();
				 returIntent.putExtra("Marca", marca);
